class LotteriesController < ApplicationController
  ...
  def create_lottery
    ...
    if type == "app"
      ...
      if feed.size < 1
        render json: { feed_count_flg: false }
      else
        ...
        if lottery.size < 1
          ...
          render json: { feed_count_flg: true, lottery_flg: 0, lottery: lottery}
        elsif lottery.size < 2
          #1번 참여
            #이벤트에 한번 참여하고, 피드에 글 있을때
            lottery = Lottery.create_lottery(tel_no, type, today, params[:user_id], 2)
            render json: {feed_count_flg: true, lottery_flg: 1, feed_data:true, lottery: lottery}
        else
          #2번이상 이벤트 참여
          render json: {feed_count_flg: true, lottery_flg: -1, lottery: lottery[0]}
        end
      end
    else
      #app이 아닌 다른곳에서 들어왔을때
